...
def test_classify_single_digit(root):
    ...
    digits = []
    for i in range(12):
        wav, _ = ta.load(f'{root}/audio_files/phone_digits_8k/phone_{i}.wav')
        digits.append(wav)
    some_digits = torch.cat([digits[1], torch.zeros(1, 3600), digits[6], torch.zeros(1, 2600), digits[3]], -1)
    classify_digit_stream(some_digits)
...
def test_classify_digit_stream(root):
    ...
    assert isinstance(digit_stream, list) and len(digit_stream) > 0 and isinstance(digit_stream[0],
                                                                                   int), "return type should be list of ints"
...
if __name__ == "__main__":
    ...
    assert isinstance(ret[0], torch.Tensor) and ret[0].dtype == torch.float32 and isinstance(ret[1],
                                                                                             int), "load_wav should return (torch.Tensor (float), int)"

    dummy = torch.randn(1, 32000)
    stft_res = do_stft(dummy, n_fft=512)
    assert isinstance(stft_res, torch.Tensor) and stft_res.shape[
        -1] == 2, "do_stft should return torch.Tensor with expected shape"

    fft_res = do_fft(dummy)
    assert isinstance(fft_res, torch.Tensor) and fft_res.dtype in {torch.complex32,
                                                                   torch.complex64}, "do_stft should return torch.Tensor with expected type"

    ret = do_istft(stft_res, n_fft=512)
    assert isinstance(ret, torch.Tensor) and ret.shape[
        -1] == 32000, "do_istft should return torch.Tensor with expected shapes"
